...
EOF
    ...
    # Expected output with all whitespace removed for easier matching
    expected_output="dsh3>CommandcannotendwithPipedsh3>cmdloopreturned2"
...
@test "ending with white space will not break shell" {
    run "./dsh" <<EOF                
cd bats     
EOF
...
@test "Pipeline with invalid command in between" {
    run "./dsh" <<EOF                
...
@test "test only whitespace input" {
    run "./dsh" <<EOF
