...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    if (!cmd_buff) return client_cleanup(0, NULL, NULL, ERR_MEMORY);

    char *rsp_buff = malloc(RDSH_COMM_BUFF_SZ);
    if (!rsp_buff) return client_cleanup(0, cmd_buff, NULL, ERR_MEMORY);

    int cli_socket = start_client(address, port);
    if (cli_socket < 0) return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_CLIENT);
    ...
    while (1) 
    {
        ...
        while ((io_size = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0)) > 0) {
            ...
            rsp_buff[is_eof ? io_size - 1 : io_size] = '\0';
