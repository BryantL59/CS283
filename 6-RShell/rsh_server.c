...
void trimChar(char *cmd) {
    ...
    while (length > 0) {
        char last_char = cmd[length - 1];
        if (last_char == SPACE_CHAR || last_char == '\t' || (last_char >= 0 && last_char <= 31)) {
            cmd[length - 1] = '\0'; 
            length--;
        } else {
            break;
...
int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        prev_rc = build_cmd_list(io_buff, &cmd_list);
        ...
        if (cmd_rc < 0) {
            send_message_string(cli_socket, "50");
        }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    pid_t pids[clist->num];         // Process IDs
    int pids_st[clist->num];        // Exit status for each process
    int exit_code = 0;
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pid == 0) {  // Child process
            // Handle input redirection for the first process
            if (i == 0) {
            ...
            // Handle output redirection for the last process
            if (i == clist->num - 1) {
    ...
    // Check if any child process exited with failure (EXIT_FAILURE)
    for (int i = 0; i < clist->num; i++) {
        if (WEXITSTATUS(pids_st[i]) == EXIT_FAILURE) {
